...
**手順**:
- サーバー起動: APIは `PORT=8001`（フロントから参照される既定）、UI配信は `PORT=8014` で確認
- ブラウザ操作（playwright-mcp）:
  1. `http://localhost:8014/` へアクセスし「新規ゲーム作成」
  2. `board.yaml`/`pieces.yaml`/`rules.yaml` を有効なテンプレート値で入力
  3. 「設定を検証」→エラーなしを目視
  4. 「ゲームを作成」→ `ゲームプレイ - <game_id>` 画面遷移を確認

**結果**:
- `GET http://localhost:8001/api/health` が `{"status":"ok"}` を返却
- 「ゲームを作成」後、`ゲームプレイ - <game_id>` が表示され、`現在のターン: プレイヤー1` を確認

**メモ**:
- フロントの既定APIベースURLは `http://localhost:8001/api` のため、APIサーバーは8001で起動必要
- 初回はYAML整形が不正（行頭ハイフン検知ロジック）で検証エラーが出たが、正しいYAMLを再入力して解消
