...
### 1.6 実装状況（MVP, 2025-08-29 現在）
- 実装済み（MVP）:
  - フロントエンドUI（トップ/ゲーム作成/参加/ゲームプレイ）
  - 盤面形状選択（矩形・クアッドスフィア）
  - YAML設定エディタ（テンプレート・インポート/エクスポート・一括ZIP）
  - 設定バリデーション（エラー/警告表示、整合性チェック）
  - ゲーム作成/取得/移動（REST API）
  - リアルタイム同期（WebSocket: join/update）
  - AI対戦（クライアントサイドAI: 簡単/普通/難しい）
- 未実装/拡張:
  - 履歴リプレイUI/観戦モード（拡張）
  - サーバー側での厳密な合法手/勝利判定（将来拡張、現行はMVP最小ロジック）
  - 認証/認可、永続化（現状はメモリ保持）
